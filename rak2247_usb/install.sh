...
GATEWAY_EUI="0102030405060708" # toupper

echo "Set the Gateway_id is $GATEWAY_EUI"
...
cp $SCRIPT_DIR/global_conf $INSTALL_DIR/packet_forwarder/lora_pkt_fwd/ -rf
cp $SCRIPT_DIR/global_conf/global_conf.eu_863_870.json $INSTALL_DIR/packet_forwarder/lora_pkt_fwd/global_conf.json

cp $SCRIPT_DIR/start.sh packet_forwarder/lora_pkt_fwd/
cp $SCRIPT_DIR/ttn-gateway.service /lib/systemd/system/
systemctl enable ttn-gateway.service

echo "RAK2247 installation is complete, you can modify the gateway_id in /opt/ttn-gateway/packet_forwarder/lora_pkt_fwd/local_conf.json."
